...
class SessionRepositoryTest {
	...
	void createSession() {
		...
		assertThat(repository.existsById(sessionId)).isFalse();
		repository.save(session);
		assertThat(repository.existsById(sessionId)).isTrue();
	}
	...
	void existsBySessionIdAndMunicipalityId() {
		...
		assertThat(repository.existsBySessionIdAndMunicipalityId(sessionId, MUNICIPALITY)).isTrue();
		assertThat(repository.existsBySessionIdAndMunicipalityId(UUID.randomUUID().toString(), MUNICIPALITY)).isFalse();
	}
	...
	void existsBySessionIdAndMunicipalityIdAndInitializedIsNotNull() {
		...
		assertThat(repository.existsBySessionIdAndMunicipalityIdAndInitializedIsNotNull(sessionNotYetInitialized, MUNICIPALITY)).isFalse();
		assertThat(repository.existsBySessionIdAndMunicipalityIdAndInitializedIsNotNull(sessionInitialized, MUNICIPALITY)).isTrue();
	}

	@Test
	void getReferenceBySessionIdAndMunicipalityId() {
		final var sessionId = "4dc21d5e-8a70-45fb-b225-367fcd383a2e";
		final var session = repository.getReferenceBySessionIdAndMunicipalityId(sessionId, MUNICIPALITY);
	...
	void findBySessionIdAndMunicipalityId() {
		...
		assertThat(repository.findBySessionIdAndMunicipalityId(sessionId, MUNICIPALITY)).isPresent();
		assertThat(repository.findBySessionIdAndMunicipalityId(UUID.randomUUID().toString(), MUNICIPALITY)).isEmpty();
	}
	...
	void findAllByMunicipalityIdAndInitializedIsNull() {
		final var matches = repository.findAllByMunicipalityIdAndInitializedIsNull(MUNICIPALITY);
	...
	void findAllByMunicipalityIdAndLastAccessedBeforeOrLastAccessedIsNull(OffsetDateTime timestamp, List<String> matchingSessionIds) {
		final var matches = repository.findAllByMunicipalityIdAndLastAccessedBeforeOrLastAccessedIsNull(MUNICIPALITY, timestamp);
	...
	void updateSession() {
		...
		final var session = repository.getReferenceById(sessionId);
		...
		final var updatedSession = repository.getReferenceById(sessionId);
	...
	void deleteSessionFile() {
		...
		final var session = repository.getReferenceById(sessionId);
		session.getFiles().removeIf(file -> Objects.equals(file.getFileId(), fileId));
		repository.save(session);

		assertThat(repository.getReferenceById(sessionId).getFiles()).isEmpty();
	}

	@Test
	void deleteSuccessfulSessionWithFiles() { // This is here to verify that that cascading delete is working properly
		final var sessionId = "158cfabe-1c3d-433c-b71f-1c909beaa291";

		assertThat(repository.existsById(sessionId)).isTrue();
		repository.deleteBySessionIdAndMunicipalityId(sessionId, MUNICIPALITY);
		assertThat(repository.existsById(sessionId)).isFalse();
	}
