...
class IntricMapperTest {
	...
	private final IntricMapper intricMapper = new IntricMapper();
	...
	void toFacilitiesFromMap() {
		...
		final var list = intricMapper.toFacilities(input);
	...
	void toFacilitiesFromNullMap() {
		// Act and assert
		assertThat(intricMapper.toFacilities((Map<String, InstalledBaseCustomer>) null)).isEmpty();
	}
	...
	void toFacilitiesFromEntry() {
		...
		final var list = intricMapper.toFacilities(Map.entry("123456", installedBaseCustomerMock));
	...
	void toFacilitiesFromNullEntry() {
		// Act and assert
		assertThat(intricMapper.toFacilities((Map.Entry<String, InstalledBaseCustomer>) null)).isEmpty();
	}
	...
	void toFacility() {
		...
		final var bean = intricMapper.toFacility(installedBaseItemMock);
	...
	void toFacilityFromNull() {
		// Act and assert
		assertThat(intricMapper.toFacility(null)).isNull();
	}
	...
	void toAddress() {
		...
		// Act
		final var bean = intricMapper.toAddress(installedBaseItemAddressMock);
	...
	void toAddressFromNull() {
		// Act and assert
		assertThat(intricMapper.toAddress(null)).isNull();
	}
	...
	void toMetadatas() {
		...
		// Act
		final var list = intricMapper.toMetadatas(input);
	...
	void addInstalledBasesWhenMatch() {
		...
		// Act
		intricMapper.addInstalledBases(facility, input);

		// Assert and verify
		assertThat(facility.getInstalledBases()).hasSize(2)
			.satisfiesOnlyOnce(ib -> assertThat(ib).isNotEqualTo(installedBase)
				.hasAllNullFieldsOrPropertiesExcept("information")
				.hasFieldOrPropertyWithValue("information", emptyList()))
			.satisfiesOnlyOnce(ib -> assertThat(ib).isEqualTo(installedBase)
	...
	void addInstalledBasesWhenNoMatch() {
		...
		// Act
		intricMapper.addInstalledBases(facility, List.of(installedBaseItemMock));

		// Assert and verify
		assertThat(facility.getInstalledBases()).hasSize(1)
			.allSatisfy(ib -> assertThat(ib).hasAllNullFieldsOrProperties())
			.satisfiesOnlyOnce(ib -> assertThat(ib).isEqualTo(installedBase));
	...
	void addInstalledBasesForNull() {
		// Act and assert
		assertDoesNotThrow(() -> intricMapper.addInstalledBases(null, null));
	}
	...
	void toInstalledBase() {
		...
		// Act
		final var bean = intricMapper.toInstalledBase(installedBaseItemMock);
	...
	void toMetadatasFromNull() {
		// Act and assert
		assertThat(intricMapper.toMetadatas(null)).isEmpty();
	}
	...
	void toMetadata() {
		...
		// Act
		final var bean = intricMapper.toMetadata(installedBaseItemMetaDataMock);
	...
	void toMetadataFromNull() {
		// Act and assert
		assertThat(intricMapper.toMetadata(null)).isNull();
	}
	...
	void toIntricModel() {
		...
		// Act
		final var result = intricMapper.toIntricModel(Map.of("123456", installedBaseCustomerMock));

		// Assert and verify
		assertThat(result.getFacilities()).hasSize(1);
		assertThat(result.getFacilities().getFirst().getInstalledBases()).hasSize(2).satisfiesExactlyInAnyOrder(ib -> {
			assertThat(ib.getType()).isEqualTo(type1);
		}, ib -> {
			assertThat(ib.getType()).isEqualTo(type2);
		});
